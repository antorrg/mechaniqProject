...
export class Auth {
  ...
  static async verifyToken (req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!token) {
        next(middError('Unauthorized access. Token not provided', 401)); return
      }
      if (token.startsWith('Bearer')) {
        token = token.slice(6).trim()
      }
      if (token === '' || token === 'null' || token === 'undefined') {
        next(middError('Missing token!', 401)); return
      }
      ...
      if (err.name === 'TokenExpiredError') {
        next(middError('Expired token', 401)); return
      }
      next(middError('Invalid token', 401))
    }
  }
  ...
  static checkRole (allowedRoles: number[]) {
    return (req: Request, res: Response, next: NextFunction) => {
      ...
      if (typeof userRole === 'number' && allowedRoles.includes(userRole)) {
        ...
        next(middError('Access forbidden!', 403))
      }
    }
  }
}
